...
def main():
  ...
  parser.add_option('--other_chr', dest='otherChr',
                    default=False, action='store_true',
                    help='Use other chr?. [Default: %default]')

  parser.add_option('--chrY', dest='chrY',
                    default=True, action='store_true',
                    help='Use other chrY?. [Default: %default]')
  ...
  if not options.restart:
    ...
    for chrom in chrom_contigs:
      if options.chromsome is not None:
        if chrom != options.chromsome:
          continue
      if options.otherChr:
        contigs += [Contig(chrom, ctg_start, ctg_end) for ctg_start, ctg_end in chrom_contigs[chrom]]
      else:
        if options.chrY:
          if (not chrom.startswith('chrUn')) and ('_' not in chrom) and (chrom!='chrM') and (chrom!='chrEBV') and (chrom!='chrMT') and ('*' not in chrom) and ('R' not in chrom) and ('Het' not in chrom) and (chrom!='chrU') and ('L' not in chrom) and ('U' not in chrom):
            contigs += [Contig(chrom, ctg_start, ctg_end) for ctg_start, ctg_end in chrom_contigs[chrom]]
        else:
          if (not chrom.startswith('chrUn')) and ('_' not in chrom) and (chrom!='chrM') and (chrom!='chrEBV') and (chrom!='chrMT') and ('*' not in chrom) and ('R' not in chrom) and ('Het' not in chrom) and (chrom!='chrU') and ('L' not in chrom) and ('U' not in chrom) and (chrom!='chrY'):
            contigs += [Contig(chrom, ctg_start, ctg_end) for ctg_start, ctg_end in chrom_contigs[chrom]]
  ...
  if not options.restart:
    ...
    for fi in range(len(fold_contigs)):
      ...
      for ctg in fold_contigs[fi]:
        line = '%s\t%d\t%d\t%s\t%d' % (ctg.chr, ctg.start, ctg.end, fold_labels[fi], i)
        print(line, file=ctg_bed_out)
        i += 1
    ctg_bed_out.close()

    chr_l_bed_file = '%s/chr_length.bed' % options.out_dir
    chr_l_bed_out = open(chr_l_bed_file, 'w')
    for fi in range(len(fold_contigs)):
      i = 1
      for ctg in fold_contigs[fi]:
        line = '%s\t%d' % (ctg.chr, ctg.end)
        print(line, file=chr_l_bed_out)
        i += 1
    chr_l_bed_out.close()
  ...
  target_length = 196608 - 2*options.crop_bp
...
def contig_sequences(contigs, seq_length, stride, snap=1, label=None):
  ...
  for ctg in contigs:
    ...
    if seq_end > ctg.end:
      mid = (ctg.start + ctg.end) // 2
      mseqs.append(ModelSeq(ctg.chr, mid-seq_length//2, mid+seq_length//2, label))
      continue
