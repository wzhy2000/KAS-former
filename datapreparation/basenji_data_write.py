...
import datapreparation.basenji_data_read as br
from basenji_data_1 import ModelSeq_2
...
def main():
  ...
  if len(args) != 5:
    ...
    fasta_file = args[0] ###fasta
    seqs_bed_file = args[1] ##sequences.bed
    seqs_cov_dir = args[2]###h5file
    proseq_id_file = args[3] ##kasseqdata # id.txt
    tfr_file = args[4] ##tfr.file  (tfr_dir, fold_set, tfr_i)
  ...
  with tf.io.TFRecordWriter(tfr_file, tf_opts) as writer:
    for si in range(num_seqs):
      ...
      kas_seq = np.asarray(get_kas_seq(proseq_id_file, mseq.chr, mseq_start, mseq_end, mseq.protype)) 
...
def get_kas_seq(kas_id_file, chr, start, end, protype):
  kas_seq = []
  new_base_path = os.path.dirname(kas_id_file)
  genome_cov_file = os.path.join(new_base_path, protype + '.bigWig')
  try:
