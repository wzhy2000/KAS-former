...
def deserialize(serialized_example, metadata):
    feature_map = {
        ...
        'kas_seq': tf.io.FixedLenFeature([], tf.string),
    ...
    kas = tf.io.decode_raw(example['kas_seq'], tf.float16)
    kas = tf.reshape(kas, (metadata['kas_length'], metadata['num_kas']))
...
def evaluate_model(model, dataset, head, max_steps=None):
    ...
    @tf.function
    def predict(x, kas):
        return model(x, kas, is_training=False)[head]
    ...
    for i, batch in enumerate(dataset):
        if max_steps is not None and i > max_steps:
            break
        predicted = predict(batch['sequence'], batch['kas_seq'])
        loss += tf.reduce_mean(
...
def create_step_function(model, optimizer):
    ...
    def train_step(batch, head, optimizer_clip_norm_global=0.2):
        with tf.GradientTape() as tape:
            outputs = model(batch['sequence'], batch['kas_seq'], is_training=True)[head]
...
def main():
    ...
    log_dir = r'../log/' + current_time + '1500_0.0001'

    max_correlation = 0

    summary_writer = tf.summary.create_file_writer(log_dir)

    human_dataset = get_dataset('kas', 'train').batch(1).repeat().prefetch(2)
    ...
    model = Enformer(channels=1536 // 2, 
                     num_heads=8,
    ...
    for epoch_i in range(num_epochs):
        ...
        metrics_human, loss_human_val = evaluate_model(model,
                                                       dataset=get_dataset('kas', 'valid').batch(1).prefetch(2),
                                                       head='human',
                                                       max_steps=1000)

        with summary_writer.as_default():
            tf.summary.scalar("huamn_PearsonR", metrics_human['PearsonR'].numpy().mean(), step=epoch_i)
            tf.summary.scalar("train_loss", loss_train / step_num, step=epoch_i)
