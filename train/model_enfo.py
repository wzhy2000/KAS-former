...
class Enformer(tf.keras.Model):
  ...
  def __init__(self,
    ...
    stem_kas = Sequential([
        tf.keras.layers.Conv1D(filters=channels // 2, kernel_size=15, padding="same"),
        Residual(conv_block(channels // 2, 1, name='pointwise_conv_block_kas')),
        pooling_module(pooling_type, pool_size=2),
    ], name='stem_kas')
    ...
    conv_tower_kas = Sequential([
        Sequential([
            conv_block(num_filters, 5),
            Residual(conv_block(num_filters, 1, name='pointwise_conv_block_kas')),
            pooling_module(pooling_type, pool_size=2),
            ],
                   name=f'conv_tower_block_{i}')
        for i, num_filters in enumerate(filter_list)], name='conv_tower_kas')

    self.concat_x_kasseq = Sequential([tf.keras.layers.Dense(channels)], name='concat_x_kas')
    ...
    self._conv_kas = Sequential([stem_kas,
                            conv_tower_kas],
                            name = 'conv_block_kas')
  ...
  def __call__(self, inputs: tf.Tensor, kas: tf.Tensor, is_training: bool) -> Dict[str, tf.Tensor]:

    outputs = self.conv(inputs, training=is_training)
    out_kas = self.conv_kas(kas, training=is_training)
    outputs = tf.concat([outputs, out_kas], axis=-1)
    outputs = self.concat_x_kasseq(outputs, training=is_training)
