...
def main():
    ...
    parser.add_option('--pro', dest='kas',
                      default='kas', type='str',
                      help='Protype of kas-seq data [Default: %default]')
    ...
    # #read kasseq sample
    #######################################
    kasseq_id = pd.read_csv(kasseq_id_file, index_col=0, sep='\t', header=None)
    num_file_paths = len(kasseq_id)
    all_seqs_bed_file = '%s/sequences_all.bed' % options.out_dir

    for i in range(num_file_paths):
        new_protype = options.kas + str(i + 1)

        with open(seqs_bed_file, 'r') as r_obj:
            sections = r_obj.readlines()
        sections = [section.strip().split()[:4] + [new_protype] for section in sections]
        with open(all_seqs_bed_file, 'a') as w_obj:
            for section in sections:
                w_obj.write('\t'.join(section) + '\n')

    train_seq, valid_seq, test_seq, train_chr22_seq, valid_chr22_seq, test_chr22_seq = first_five(all_seqs_bed_file)
    ...
    with open(seqs_bed_file, 'w') as w_obj:
        for item in train_seq:
            w_obj.write(item['chr'] + '\t' + item['start'] + '\t' + item['end'] + '\t' + 'train' + '\t' + item[
                'protype'] + '\n')
        for item in valid_seq:
            w_obj.write(item['chr'] + '\t' + item['start'] + '\t' + item['end'] + '\t' + 'valid' + '\t' + item[
                'protype'] + '\n')
        for item in test_seq:
            w_obj.write(
                item['chr'] + '\t' + item['start'] + '\t' + item['end'] + '\t' + 'test' + '\t' + item['protype'] + '\n')
...
# -------------------------------------------------------------------------------------------------
    ...
    for ti in range(targets_df.shape[0]):
        ...
        if options.restart and os.path.isfile(seqs_cov_file):
            ...
            cmd = 'python basenji_data_read.py' 
    ...
    for fold_set in fold_labels:
        ...
        while tfr_start <= fold_set_end:
            ...
            cmd = 'python basenji_data_write.py'
            cmd += ' -s %d' % tfr_start
            cmd += ' -e %d' % tfr_end
    ...
    stats_dict['num_kas'] = 1

    stats_dict['kas_length'] = (options.seq_length + 2 * 32768)  # // options.pool_width
